	...
	public GUI() {
		...
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				RS232.getInstance().schliesseSerialPort();
				System.exit(0);
			}
		});
		...
		button_calculateGraphs.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				RS232.getInstance().sendeSerialPort(Arrays.copyOfRange(sequenceData, 0, 2));
				try {
					Thread.sleep(500);
				...
				RS232.getInstance().sendeSerialPort(Arrays.copyOfRange(sequenceData, 2, 4));
				try {
					Thread.sleep(500);
				} catch (InterruptedException e1) {
					// TODO Auto-generated catch block
				...
				RS232.getInstance().sendeSerialPort(Arrays.copyOfRange(sequenceData, 4, sequenceData.length));
			}
		});
		...
		textfield_sequenceLength = new JTextField("Länge in Sek (1s def.)");
		...
		slider_red.setBounds(25, 3 * dimensionGraph.height + 100, dimensionGraph.width, 25);
		slider_red.setBackground(Color.RED);
		slider_red.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				if (!slider_red.getValueIsAdjusting())
		...
		slider_yellow.setBounds(25, 3 * dimensionGraph.height + 125, dimensionGraph.width, 25);
		slider_yellow.setBackground(Color.YELLOW);
		slider_yellow.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				if (!slider_yellow.getValueIsAdjusting())
					sendIndividualBrightness(slider_red.getValue(), slider_yellow.getValue(), slider_green.getValue());
			}
		});
		...
		slider_green.setBounds(25, 3 * dimensionGraph.height + 150, dimensionGraph.width, 25);
		slider_green.setBackground(Color.GREEN);
		slider_green.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				if (!slider_green.getValueIsAdjusting())
					sendIndividualBrightness(slider_red.getValue(), slider_yellow.getValue(), slider_green.getValue());
			}
		});
